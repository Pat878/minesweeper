    ...
    $(".divTableCell").on("click", function() {
      for (var i = 0; i < mineArray.length; i++) {
        if ($(this).attr("data") == mineArray[i]) {
          for (var j = 0; j < 81; j++) {
            $('*[data="' + mineArray[j] + '"]')
              .html('<i class="fa fa-bomb" aria-hidden="true"></i>')
              .addClass("open mine")
              .removeClass("closed fa-flag");
            $('*[data="' + j + '"]')
              .addClass("open")
              .removeClass("closed");
            clearInterval(timer);
...
var revealCells = (function() {
  return function() {
    $(".divTableCell").on("click", function() {
      ...
      if (
        distanceToMineArray[theNumber] > 0 &&
        $(this).hasClass("number") !== true &&
        $(this).hasClass("mine") !== true &&
        $(this).hasClass("fa-flag") == false
      ) {
        $(this)
          .addClass("open number")
          .append(distanceToMineArray[theNumber])
          .removeClass("closed");
      }

      if (
        distanceToMineArray[theNumber] == 0 &&
        $(this).hasClass("click") == false &&
        $(this).hasClass("fa-flag") == false
      ) {
        $(this)
          .addClass("open click")
        ...
        if (
          thisCell > 0 &&
          thisCell < 81 &&
          openCells.includes(thisCell) == false
        ) {
        ...
        function reveal() {
          for (var i = 0; i < numbers.length; i++) {
            ...
            if (
              thisNum > 0 &&
              thisNum < 81 &&
              openCells.includes(thisNum) == false
            ) {
        ...
        function open() {
          for (var o = 0; o < openCells.length; o++) {
            ...
            while (
              $(clickboy).hasClass("click") == false &&
              $(clickboy).hasClass("number") == false &&
              $(clickboy).hasClass("fa-flag") == false
            ) {
        ...
        function showNumbers() {
          $(".open").each(function(i, obj) {
            ...
            if (
              distanceToMineArray[num] > 0 &&
              $(this).hasClass("number") !== true &&
              $(this).hasClass("mine") !== true
            ) {
              $(this)
                .append(distanceToMineArray[$(this).attr("data")])
                .addClass("number")
                .removeClass("closed");
            }
          });
...
var addFlag = (function() {
  return function() {
    function flagger() {
      $(".divTableCell").on("contextmenu", function(e) {
        return false;
      });
...
function screenWidth() {
  $(window).resize(function() {
    if ($(window).width() < 960) {
      $(".container").hide();
      $("body").html(
        "<center><p class='bg-danger'>Sorry! Your device's screen is too small to play this game! If you'd like to play, your screen must be larger than 960 pixels, and you'll need the ability to right-click.</p></center>"
      );
    } else {
      $(".container").show();
      location.reload();
